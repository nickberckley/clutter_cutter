...
class OUTLINER_MT_purge_by_type(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        mid1.operator('outliner.purge_orphan_mesh', text='Mesh' + ' (' + str(meshcount) + ')', icon='MESH_DATA')
        if meshcount < 1:
            mid1.enabled = False

        mid2 = grid.row()
        curvecount = orphaned_counter('curves')
        mid2.operator('outliner.purge_orphan_curve', text='Curve' + ' (' + str(curvecount) + ')', icon='CURVE_DATA')
        if curvecount < 1:
            mid2.enabled = False

        mid3 = grid.row()
        greasepencilcount = orphaned_counter('grease_pencils')
        mid3.operator('outliner.purge_orphan_grease', text='Grease Pencil' + ' (' + str(greasepencilcount) + ')', icon='GREASEPENCIL')
        if greasepencilcount < 1:
            mid3.enabled = False

        mid4 = grid.row()
        metaballcount = orphaned_counter('metaballs')
        mid4.operator('outliner.purge_orphan_metaball', text='Metaball' + ' (' + str(metaballcount) + ')', icon='META_DATA')
        if metaballcount < 1:
            mid4.enabled = False

        mid5 = grid.row()
        haircount = orphaned_counter('hair_curves')
        mid5.operator('outliner.purge_orphan_hair', text='Hair' + ' (' + str(haircount) + ')', icon='OUTLINER_DATA_CURVES')
        if haircount < 1:
            mid5.enabled = False

        mid6 = grid.row()
        volumecount = orphaned_counter('volumes')
        mid6.operator('outliner.purge_orphan_volume', text='Volume' + ' (' + str(volumecount) + ')', icon='OUTLINER_DATA_VOLUME')
        if volumecount < 1:
            mid6.enabled = False

        mid7 = grid.row()
        latticecount = orphaned_counter('lattices')
        op = mid7.operator('outliner.purge_unrecursive', text='Lattice' + ' (' + str(latticecount) + ')', icon='LATTICE_DATA')
        op.data_type="lattices"
        if latticecount < 1:
            mid7.enabled = False

        mid8 = grid.row()
        speakercount = orphaned_counter('speakers')
        op = mid8.operator('outliner.purge_unrecursive', text='Speaker' + ' (' + str(speakercount) + ')', icon='OUTLINER_DATA_SPEAKER')
        op.data_type="speakers"
        if speakercount < 1:
            mid8.enabled = False

        mid9 = grid.row()
        lightprobecount = orphaned_counter('lightprobes')
        op = mid9.operator('outliner.purge_unrecursive', text='Light Probe' + ' (' + str(lightprobecount) + ')', icon='OUTLINER_DATA_LIGHTPROBE')
        op.data_type="lightprobes"
        ...
        op = bot1.operator('outliner.purge_unrecursive', text='Action' + ' (' + str(actioncount) + ')', icon='ACTION')
        op.data_type="actions"
        if actioncount < 1:
            bot1.enabled = False

        bot2 = grid.row()
        armaturecount = orphaned_counter('armatures')
        op = bot2.operator('outliner.purge_unrecursive', text='Armature' + ' (' + str(armaturecount) + ')', icon='ARMATURE_DATA')
        op.data_type="armatures"
        if armaturecount < 1:
            bot2.enabled = False

#        bot3 = grid.row()
#        shapekeycount = orphaned_counter('shape_keys')
#        op = bot3.operator('outliner.purge_unrecursive', text='Shape Keys' + ' (' + str(shapekeycount) + ')', icon='SHAPEKEY_DATA')
#        op.data_type="shapekeys"
#        if shapekeycount < 1:
#            bot3.enabled = False

        bot4 = grid.row()
        cameracount = orphaned_counter('cameras')
        op = bot4.operator('outliner.purge_unrecursive', text='Camera' + ' (' + str(cameracount) + ')', icon='CAMERA_DATA')
        op.data_type="cameras"
        if cameracount < 1:
            bot4.enabled = False

        bot5 = grid.row()
        lightcount = orphaned_counter('lights')
        op = bot5.operator('outliner.purge_unrecursive', text='Light' + ' (' + str(lightcount) + ')', icon='LIGHT_DATA')
        op.data_type="lights"
        if lightcount < 1:
            bot5.enabled = False
        ...
        op = bot6.operator('outliner.purge_unrecursive', text='Movie Clip' + ' (' + str(movieclipcount) + ')', icon='TRACKER')
        op.data_type="movieclips"
        if movieclipcount < 1:
            bot6.enabled = False

        bot7 = grid.row()
        soundcount = orphaned_counter('sounds')
        op = bot7.operator('outliner.purge_unrecursive', text='Sound' + ' (' + str(soundcount) + ')', icon='FILE_SOUND')
        op.data_type="sounds"
        if soundcount < 1:
            bot7.enabled = False
        ...
        op = bot8.operator('outliner.purge_unrecursive', text='Text' + ' (' + str(textcount) + ')', icon='TEXT')
        op.data_type="texts"
        if textcount < 1:
            bot8.enabled = False
        ...
        op = bot9.operator('outliner.purge_unrecursive', text='Font' + ' (' + str(fontcount) + ')', icon='FONT_DATA')
        op.data_type="fonts"
        if fontcount < 1:
            bot9.enabled = False
